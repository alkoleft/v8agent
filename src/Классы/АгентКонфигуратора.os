...
Функция Запустить() Экспорт
	...
	Конфигуратор.УстановитьКонтекст(ПараметрыАгента.СтрокаПодключения, ПараметрыАвторизации.Пользователь, ПараметрыАвторизации.Пароль);
...
Функция ЗавершитьРаботуАгента() Экспорт
	...
	Результат = ВыполнитьКомандуАгент("common shutdown", "Выключение агента", Ложь);
